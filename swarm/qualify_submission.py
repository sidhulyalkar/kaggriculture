...
SUBMISSION_GRADE_SCOPES = {"verified_v32_public_frontier", "verified_public_frontier"}
...
def _promoted_candidates(campaign_root: Path) -> list[dict[str, Any]]:
    ...
    for epoch in sorted((campaign_root / "epochs").glob("epoch-*")):
        ...
        candidates = {str(row["candidate_id"]): row for row in json.loads(candidates_path.read_text(encoding="utf-8"))}
        ...
        for decision in evaluation.get("decisions", []):
            ...
            candidate = candidates.get(candidate_id); evidence = heldout.get(candidate_id)
...
def _verify_archive(archive: Path) -> dict[str, Any]:
    check = subprocess.run([sys.executable, "scripts/check_submission.py", str(archive)], capture_output=True, text=True, timeout=120)
    result: dict[str, Any] = {
    ...
    result["post_pack_smoke"] = smoke; result["ok"] = bool(smoke["ok"])
    return result


def qualify_campaign(
    *, campaign_root: str | Path, config_path: str, champion_path: str,
    output_root: str | Path, frontier_scope: str, max_confirmation_candidates: int = 3,
) -> dict[str, Any]:
    campaign = Path(campaign_root); output = Path(output_root); output.mkdir(parents=True, exist_ok=True)
    config = load_config(config_path); thresholds = config["experiments"]["promotion"]
    promoted = _promoted_candidates(campaign)
    result: dict[str, Any] = {
        "frontier_scope": frontier_scope,
        "submission_grade_scope": frontier_scope in SUBMISSION_GRADE_SCOPES,
    ...
    if not promoted:
        (output / "SUBMISSION_DECISION.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8"); return result

    qualified: list[dict[str, Any]] = []
    for candidate_row in promoted[: max(1, int(max_confirmation_candidates))]:
        candidate_id = candidate_row["candidate_id"]; candidate = candidate_row["candidate"]
        source_path = Path(str(candidate["source_path"])); lane = str(candidate.get("lane", ""))
        raw = evaluate_candidate(candidate_path=str(source_path), champion_path=champion_path, seeds=list(CONFIRMATION_SEEDS), both_seats=True, stage="confirmation")
        record = _evaluation_record(raw, candidate_id=candidate_id); decision = promotion_decision(record, thresholds, lane=lane)
        row = {"candidate_id": candidate_id, "lane": lane, "heldout": candidate_row["heldout"], "confirmation": raw, "confirmation_decision": decision.to_dict()}
        result["confirmation"].append(row)
        if decision.promote: qualified.append(row)
    ...
    winner = qualified[0]; candidate_id = str(winner["candidate_id"])
    source_path: Path | None = None
    for candidate_row in promoted:
        if candidate_row["candidate_id"] == candidate_id:
            source_path = Path(str(candidate_row["candidate"]["source_path"])); break
    if source_path is None: raise RuntimeError(f"qualified candidate source disappeared: {candidate_id}")

    archive = output / f"NEXT_SUBMIT_SWARM_{candidate_id}.tar.gz"
    _package_tree(source_path.parent, archive); archive_verification = _verify_archive(archive)
    result["winner"] = winner; result["archive"] = str(archive); result["archive_verification"] = archive_verification

    if frontier_scope not in SUBMISSION_GRADE_SCOPES:
        result["decision"] = "HOLD"; result["reason"] = "candidate qualified but frontier control scope was not submission-grade"
    elif not archive_verification.get("ok"):
        result["decision"] = "HOLD"; result["reason"] = "candidate qualified scientifically but final packaged archive failed runtime verification"
    else:
        result["decision"] = "PROMOTE"; result["reason"] = "heldout promotion replicated on sealed confirmation against verified frontier and packaged runtime passed"
