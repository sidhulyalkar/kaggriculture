...
def _tree_hash(path: str | Path) -> str:
    root = Path(path)
    if root.is_file(): return sha256(root.read_bytes()).hexdigest()
    if not root.exists(): raise FileNotFoundError(root)
    digest = sha256()
    for file_path in sorted(p for p in root.rglob("*") if p.is_file() and "__pycache__" not in p.parts and p.suffix != ".pyc"):
        relative = file_path.relative_to(root).as_posix().encode("utf-8"); data = file_path.read_bytes()
...
def _configure_provider_runtime(request: dict[str, Any]) -> None:
    ...
    if attempts < 1 or attempts > 5: raise ValueError("provider_attempts must be between 1 and 5")
    os.environ["SWARM_NVIDIA_ATTEMPTS"] = str(attempts)
    if "temperature" in request: os.environ["SWARM_NVIDIA_TEMPERATURE"] = str(float(request["temperature"]))


def _verify_frontier_pins(request: dict[str, Any], acquisition: dict[str, Any]) -> None:
    expected = request.get("frontier_expected_archive_sha256")
    if not expected: return
    if not isinstance(expected, dict): raise ValueError("frontier_expected_archive_sha256 must be a mapping")
    resources = acquisition.get("resources", {})
    for family, digest in expected.items():
        row = resources.get(str(family), {})
        actual = str(row.get("archive_sha256", ""))
        if row.get("status") != "ready" or actual != str(digest):
            raise RuntimeError(f"frontier pin mismatch for {family}: expected {digest}, got {actual or row.get('status')}")


def _acquire_if_requested(request: dict[str, Any], root: Path) -> dict[str, Any] | None:
    if not bool(request.get("auto_frontier", False)): return None
    keys = request.get("frontier_keys")
    if keys is not None and not isinstance(keys, list): raise ValueError("frontier_keys must be a list")
    acquisition = acquire_frontier(output_root=root / "frontier", keys=[str(key) for key in keys] if keys else None)
    _verify_frontier_pins(request, acquisition)
    return acquisition
...
def _resolve_experiment_scope(request: dict[str, Any], root: Path, acquisition: dict[str, Any] | None) -> tuple[str, dict[str, str], str, str | None]:
    ...
    if acquisition:
        ...
        for family in acquisition.get("ready_public_families", []):
            ...
            if row.get("status") == "ready": opponents[str(family)] = str(row["agent_root"])
    requested = request.get("opponents")
    if requested:
        if not isinstance(requested, dict): raise ValueError("opponents must be a mapping of family name to path")
...
def execute_request(*, request_path: str, output_root: str) -> dict[str, Any]:
    ...
    if not bool(request.get("enabled", False)): raise RuntimeError("live request is not enabled")
    request_id = str(request.get("request_id", "unnamed")); mode = str(request.get("mode", "probe")).lower(); config_path = str(request.get("config", "swarm/config/nvidia_live.yaml"))
    root = Path(output_root).resolve(); root.mkdir(parents=True, exist_ok=True); _configure_provider_runtime(request)
    ...
    if mode == "probe":
        ...
        acquisition = acquire_frontier(output_root=root / "frontier", keys=[str(key) for key in request.get("frontier_keys", [])] or None); _verify_frontier_pins(request, acquisition); result["frontier"] = acquisition
    elif mode in {"epoch", "campaign"}:
        acquisition = _acquire_if_requested(request, root); champion_path, opponents, frontier_scope, champion_family = _resolve_experiment_scope(request, root, acquisition); champion_hash = _tree_hash(champion_path)
        expected = str(request.get("champion_sha256", "")).strip()
        if expected and expected != champion_hash: raise RuntimeError(f"champion hash mismatch: expected {expected}, got {champion_hash}")
        epochs = 1 if mode == "epoch" else int(request.get("epochs", 1))
        if epochs < 1 or epochs > 5: raise ValueError("epochs must be between 1 and 5 for live GitHub execution")
    ...
    (root / "LIVE_REQUEST_RESULT.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8"); return result


def main() -> None:
    parser = argparse.ArgumentParser(description="Execute a secret-safe GitHub-hosted Kaggriculture swarm request"); parser.add_argument("--request", default="swarm_requests/ACTIVE.json"); parser.add_argument("--output-root", default="swarm/runs/live"); args = parser.parse_args(); result = execute_request(request_path=args.request, output_root=args.output_root); print(json.dumps(result, indent=2, sort_keys=True))
